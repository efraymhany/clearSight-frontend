...
//     console.log("Fetched Doctor Info:", foundDoctor);
//   }, [docId, doctors]);
 ...
 const [slotTime, setSlotTime] = useState("");
  ...
  const getavailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      setDocSlots((prev) => [...prev, timeSlots]); // ✅ Correctly updating state
    }
  };
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <div className="flex items-center gap-3 w-full overflow-scroll mt-4 ">
            {docSlots.length && docSlots[slotIndex].map((item , index) =>(
              <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? ' bg-primary text-white' : 'text-gray-500 border border-gray-500'}  `} key={index}>
